<template>
  <transition name="ivu-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
      v-show="visible"
      class="ivu-time-panel ivu-popper"
      :class="popperClass">
      <div class="ivu-time-panel__content" :class="{ 'has-seconds': showSeconds }">
        <time-spinner
          ref="spinner"
          @change="handleChange"
      ...
      <div class="ivu-time-panel__footer">
        ...
        <button
          type="button"
          class="ivu-time-panel__btn"
          :class="{confirm: !disabled}"
          @click="handleConfirm()">{{ t('i.datepicker.confirm') }}</button>
      </div>
    </div>
  </transition>
</template>
